...
describe('MessagesService', () => {
  ...
  describe('addReaction', () => {
    it('should add new reaction to message', async () => {
      const message = MessageFactory.build({ reactions: [] });
      const emoji = '👍';
      ...
      await service.addReaction(message.id, emoji, userId);
    ...
    it('should add user to existing reaction', async () => {
      const existingReaction = { emoji: '👍', userIds: ['user-1'] };
      const message = MessageFactory.build({ reactions: [existingReaction] });
      const userId = 'user-2';

      mockDatabase.message.findUniqueOrThrow.mockResolvedValue(message);
      mockDatabase.message.update.mockResolvedValue(message);

      await service.addReaction(message.id, '👍', userId);
    ...
    it('should not add duplicate user reaction', async () => {
      const existingReaction = { emoji: '👍', userIds: ['user-1'] };
      const message = MessageFactory.build({ reactions: [existingReaction] });
      ...
      await service.addReaction(message.id, '👍', 'user-1');
    ...
    it('should handle errors when adding reaction', async () => {
      mockDatabase.message.findUniqueOrThrow.mockRejectedValue(
        new Error('Message not found'),
      );

      await expect(
        service.addReaction('msg-id', '👍', 'user-id'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('removeReaction', () => {
    it('should remove user from reaction', async () => {
      ...
      const message = MessageFactory.build({ reactions: [reaction] });

      mockDatabase.message.findUniqueOrThrow.mockResolvedValue(message);
      mockDatabase.message.update.mockResolvedValue(message);

      await service.removeReaction(message.id, '👍', 'user-1');
    ...
    it('should remove reaction entirely when last user removed', async () => {
      const reaction = { emoji: '👍', userIds: ['user-1'] };
      const message = MessageFactory.build({ reactions: [reaction] });
    ...
    it('should handle errors when removing reaction', async () => {
      mockDatabase.message.findUniqueOrThrow.mockRejectedValue(
        new Error('Message not found'),
      );

      await expect(
        service.removeReaction('msg-id', '👍', 'user-id'),
      ).rejects.toThrow();
    });
  });
